...
Создан примитивный TestActor, который имеет сферу коллизиии и отрисавывается с помощью debug-линий. TestActor имеет 2 состояния - свободный и пересеченный игроком.
...
Для демонстрации паттерна использованы стандартные инструменты UE4.
В классе TestActor создана Сигнатура и Переменная. Также при смене состояния вызывается обработчик для подписчиков на событие.

В классе ASkillboxPatternsCharacter в методе BeginPlay() призводится подписка на собитие смены состояния сфер на карте.
При пересечении игроком сферы происходит тряска камеры.
